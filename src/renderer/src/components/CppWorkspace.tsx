...
export default function CppWorkspace(): JSX.Element {
  ...
  const submit = async (): Promise<void> => {
    ...
    if (!response.ok) {
      setSubmitStatus(response.message ?? 'Submission failed')
      setBusy(null)
    } else {
      setSubmitStatus('Submitted · waiting for evaluation...')
    }
  }

  const statement = problem?.statement_html
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-slate-100">
      ...
      <div className="flex min-h-0 flex-1 gap-3 p-3">
        <section className="flex min-w-0 flex-[1.25] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center border-b border-slate-200 px-4 py-2 text-xs font-semibold text-slate-500">
            main.cpp
            {(ioFiles.inputFile || ioFiles.outputFile) && (
              <span className="ml-3 rounded-full bg-sky-50 px-2 py-0.5 text-sky-700">
                Files: {ioFiles.inputFile ?? 'stdin'} → {ioFiles.outputFile ?? 'stdout'}
              </span>
            )}
          ...
          <textarea
            ...
            spellCheck={false}
            className="min-h-0 flex-1 resize-none bg-white p-4 font-mono text-sm leading-6 text-slate-800 outline-none selection:bg-brand-100"
          />
        ...
        <aside className="min-w-0 flex-1 overflow-y-auto rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-lg font-bold text-slate-900">
            {problem ? `#${problem.id} · ${problem.title ?? problem.slug ?? 'Problem'}` : 'Select a problem'}
          </h2>
          {!submitMatchesBrowser && problem && (
            <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
              Submit is locked. Open #{problem.id} in the pbinfo browser first.
            </div>
          )}
          {statement ? (
            <div className="statement-html text-sm text-slate-700" dangerouslySetInnerHTML={{ __html: statement }} />
          ) : (
            <div className="rounded-xl border border-dashed border-slate-300 p-5 text-sm text-slate-500">
              No cached statement. Open the problem once in pbinfo to capture it.
            </div>
          )}
        </aside>
      </div>
