...
export function cleanSourceCode(source: string): string {
  ...
  const numberedSource = extractNumberedSource(lines)
  if (numberedSource) {
    lines = numberedSource
  } else {
    ...
    if (
      standaloneNumbers.length >= 3 &&
      standaloneNumbers.length >= Math.floor(lines.length * 0.35)
    ) {
...
/** pbinfo prefixes every rendered editor line with its index, separated only
 * by an invisible zero-width character. Strip one consecutive 1..N sequence. */
function extractNumberedSource(lines: string[]): string[] | null {
  ...
  const numbered = parsed.filter(
    (item): item is { number: number; code: string } => item !== null
  )
  if (!isConsecutive(numbered.map((item) => item.number))) return null
  return numbered.map((item) => item.code)
}
